...
flags.DEFINE_string("task_name", "SimProcessor", "The name of the task to train.")
...
class SimProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train_process.csv"))[:100], "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev_process.csv"))[:100], "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test_process.csv"))[:100], "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == "test":
                ...
                text_b = tokenization.convert_to_unicode(line[0] + line[2])
                label = tokenization.convert_to_unicode(line[3])
            else:
                if len(line) != 4:
                    tf.logging.info("数据格式错误~~~")
                    continue
                text_a = tokenization.convert_to_unicode(line[0] + line[1])
                text_b = tokenization.convert_to_unicode(line[0] + line[2])
                label = tokenization.convert_to_unicode(line[3])
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    label_id = label_map[int(float(example.label))]
    if ex_index < 5:
